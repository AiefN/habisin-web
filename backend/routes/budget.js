...
// Import recipes data (same as above)
const recipes = [
  {
    id: "1",
...
router.post("/", (req, res) => {
  ...
  if (!budget || typeof budget !== "number") {
    return res.status(400).json({ error: "Budget harus berupa angka" });
  }

  // Filter recipes that fit within budget
  const affordableRecipes = recipes.filter(recipe => recipe.pricePerServing <= budget);

  res.json({
    budget,
    recipes: affordableRecipes,
    count: affordableRecipes.length
  });
});
